            ...
            for k, v in pairs(Weeds) do
                ...
                if dst < 2.5 then
                    if IsControlJustReleased(0,244) then
                        notify(Config.Langs["DestroyWeed"])
                        Destroy(k)
                    end
                end
...
function Harvest(id)
    ...
    if Weeds[id].growth >= 30 then 
        ...
        TriggerServerEvent("ambr4_weed:Harvest", GiveWeed)
...
RegisterNUICallback("Give", function (data)
    if data.idx == "Harvest" then
        ...
        TriggerServerEvent("ambr4_weed:giveSV", MenuWeed, data.idx)
    end
end)

AddEventHandler("ambr4_weed:giveCl")
RegisterNetEvent("ambr4_weed:giveCl", function (MenuW,idx)
    if idx == "Water" then
...
AddEventHandler("ambr4_weed:Seed")
RegisterNetEvent("ambr4_weed:Seed", function ()
    StartSeed()
end)
